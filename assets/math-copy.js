 ...
 */
(() => {
	if (window.ComposerBlogSettings?.math?.copy === false) return;
	...
	function writeClipboard(text) {
		...
		try {
			if (!document.execCommand("copy")) throw Error('Clipboard copy denied');
	...
	function markCopied(formula) {
		formula.setAttribute('aria-label','已复制 '+formula.dataset.copyFormat);
		formula.classList.add(COPIED_CLASS);
		window.setTimeout(() => {
			formula.classList.remove(COPIED_CLASS);
			formula.setAttribute('aria-label','复制公式（'+formula.dataset.copyFormat+'）');
		}, RESET_DELAY);
	}
	...
	function copyFormula(formula) {
		...
		writeClipboard(source)
			...
			.catch((error) => {
				formula.setAttribute('aria-label','复制失败，请选择公式复制');formula.title='复制失败，请选择公式复制';
				console.error("Failed to copy math formula: ", error);
			});
	}

	function initFormula(formula) {
		if (formula.classList.contains(COPYABLE_CLASS)) return;
		formula.dataset.copyFormat=formula.querySelector('math')?'MathML':formula.querySelector('svg')?'SVG':'文本';
		formula.setAttribute('role','button');formula.setAttribute('aria-label','复制公式（'+formula.dataset.copyFormat+'）');formula.title='复制公式（'+formula.dataset.copyFormat+'）';
