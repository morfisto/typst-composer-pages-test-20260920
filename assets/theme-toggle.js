 ...
 */
(() => {
	const settings = window.ComposerBlogSettings?.theme ?? {};
	...
	function getCurrentTheme() {
		...
		return settings.default && settings.default !== "system" ? settings.default : getSystemTheme();
	}
	...
	function updateToggleButton(theme) {
		...
		if (theme === "dark") {
			...
			button.setAttribute("aria-label", settings.lightLabel ?? "切换到浅色模式");
			button.innerHTML = ICONS.sun;
		} else {
			button.classList.remove("is-dark");
			button.setAttribute("aria-label", settings.darkLabel ?? "切换到深色模式");
	...
	function onDOMReady() {
		document.addEventListener('composer-theme-restored',event=>{if(['light','dark'].includes(event.detail))applyTheme(event.detail);});
		const button = document.getElementById("theme-toggle");
		if (button && settings.button !== false) {
		...
		window
			...
			.addEventListener("change", (e) => {
				if (!getStoredTheme() && (!settings.default || settings.default === "system")) {
